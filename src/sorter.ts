...
export class Sorter {
  ...
  generateSortPipeline(sortQuery?: string, willYouProject?: boolean, defaultSort: { [key: string]: 1 | -1 } = { _id: 1 }, log = false): PipelineStage[] {
    if (!this.config) throw new Error('The sorting config is not applied yet!')
    ...
    for (const field of validSortFields) {
      ...
      switch (config[fieldName].type) {
        case 'string':
          needSetStage = 1
          setStage[`${databaseKey}_1`] = {
            $toLower: (nullReplacementKey || nullReplacementValue)
              ? { $ifNull: [`$${databaseKey}`, `$${nullReplacementKey}`, nullReplacementValue] }
              : `$${databaseKey}`
          }
        ...
        case 'array':
          needSetStage = 1
          setStage[`${databaseKey}_1`] = {
            $size: (nullReplacementKey || nullReplacementValue)
              ? { $ifNull: [`$${databaseKey}`, `$${nullReplacementKey}`, nullReplacementValue] }
              : `$${databaseKey}`
          }
          sortStage[`${databaseKey}_1`] = field.charAt(0) === '-' ? -1 : 1
